...
rdd_final2=rdd_final.map(lambda x : (x[0],(x[1][0]).tolist())) #prepare le format pour passer en df
df_fin=sqlContext.createDataFrame(rdd_final2,['adresse','image_transformee'])
df_fin.write.parquet('s3://oliviergollnick/images_reduites_parquet',mode='overwrite')
# pour lire spark.read.parquet('s3://oliviergollnick/images_reduites_parquet')
rdd_im=df_fin.select('image_transformee').rdd
rdd_im2=rdd_im.map(lambda x : x[0])
rdd_im2.saveAsTextFile('s3://oliviergollnick/images_reduites_csv')
# pour lire spark.read.csv('s3://oliviergollnick/images_reduites_csv')